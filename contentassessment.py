...
def main():
    ...
    if st.session_state.analyses:
        ...
        for url, analysis in st.session_state.analyses.items():
            st.write(f"**URL:** {url}")
            user_primary_color = st.selectbox("Select Primary Color:", list(color_profiles.keys()), key=url + "1", index=list(color_profiles.keys()).index(analysis["primary_color"]))
            user_supporting_colors = st.multiselect("Select Supporting Colors:", list(color_profiles.keys()), key=url + "2", default=analysis["supporting_colors"].split(', '))
            user_rationale = st.text_area("Rationale:", value=analysis["rationale"], key=url + "3")
            st.session_state.analyses[url] = {"primary_color": user_primary_color, "supporting_colors": ', '.join(user_supporting_colors), "rationale": user_rationale}
