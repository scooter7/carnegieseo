    ...
    if st.button("Assess Content Colors"):
        if not urls or len(urls) > 20:
            ...
            for url in urls:
                ...
                user_supporting_colors = st.multiselect("Select Supporting Colors:", list(color_profiles.keys()), key=url + "2", default=supporting_colors.split(', '))
                user_rationale = st.text_area("Rationale:", value=rationale, key=url + "3")
            ...
            color_count_df = pd.DataFrame(list(st.session_state.color_count.items()), columns=['Color', 'Count'])
